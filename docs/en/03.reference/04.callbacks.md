...
The `registered` after all addons have registered and booted. This addon callback is fired in the order in which addons were loaded.

```php
$module->on('registered', function() {
    // Do something here after all addons are fully initialized.
});
```
