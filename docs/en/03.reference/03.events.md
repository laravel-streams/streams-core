...
This event is fired just **before** the `php artisan build` command.

```php
app('events')->dispatch(new \Anomaly\Streams\Platform\Application\Event\SystemIsBuilding());
```

### SystemHasBuilt

This event is fired just **after** the `php artisan build` command.

```php
app('events')->dispatch(new \Anomaly\Streams\Platform\Application\Event\SystemHasBuilt());
```

### SystemIsRefreshing

This event is fired just **before** the `php artisan refresh` command.

```php
app('events')->dispatch(new \Anomaly\Streams\Platform\Application\Event\SystemIsRefreshing($command));
```

### SystemHasBuilt

This event is fired just **after** the `php artisan refresh` command.

```php
app('events')->dispatch(new \Anomaly\Streams\Platform\Application\Event\SystemHasRefreshed($command));
```
