...
> The **register** and **boot** method are both called with the service container and support method injection.{.notice}
...
> For more information on route definitions checkout the [documentation on routing](#the-basics/routing).{.tip}
...
> You can also dictate the listener's priority by including a priority value. Listeners are ran in order of highest to lowest priority.{.tip}
