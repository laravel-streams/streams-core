...
> Actions extend UI buttons so some actions may use registered buttons to further automate themselves.{.tip}
...
> The Ajax API is still being developed. While ajax forms are usable, more robust JSON response information is still missing.{.notice}
...
> Handlers are called using the **Service Container** and support class and method injection.{.tip}
